...
public class MQTTFrame implements ActionListener, MqttCallback, Runnable {
    ...
    protected static final int FRAME_WIDTH = 455;
	...
	private static Properties getMQTTPrps()
	{
		Properties  props = new Properties();
		java.io.InputStream propStream = null;
		// Does a properties file exist from which the GUI can be populated?
        try {
            propStream = new java.io.BufferedInputStream(new FileInputStream( PROP_FILE ));
        ...
        {
        	try{propStream.close(); propStream = null;}catch(Exception e){}
        }
        
        return props;
	}
	
	private static void setMQTTPrps(final MQTTFrame view)
	{
		...
		java.io.OutputStream propStream = null;
		Properties props = new Properties();
		
		Properties currentSettings = getMQTTPrps();
            
        String prpKey = null; 
        
        for(java.util.Enumeration keyPrps = currentSettings.propertyNames(); keyPrps.hasMoreElements();)
        {
        	prpKey = (String)keyPrps.nextElement();
        	
        	//if(prpKey.equals("IPAddressList"));
        	//else if(prpKey.equals("IPPortList"));
        	//else if(prpKey.equals("ClientId"));
        	//else if(prpKey.equals("Persistence"));
        	//else if(prpKey.equals("PersistenceDir"));
        	//else 
        	props.setProperty(prpKey, currentSettings.getProperty(prpKey) );
        }
		
        try {
            propStream = new java.io.BufferedOutputStream(new FileOutputStream( PROP_FILE ));
            ...
            // Now write to disk
            props.store( propStream, "MQTT Utility properties" );
	...
	protected void init( Container contentPane ) {
        ...
        chkTls = new JCheckBox(); 
        chkTls.setActionCommand("SecureConnection");
        chkTls.setSelected(true); 
        chkTls.addActionListener(this);
    ...
    private MqttConnectOptions getMqttConnectOptions(final boolean isTlsCheck)
    {
    	MqttConnectOptions opts = new MqttConnectOptions();
    	int keepalive = 60;
		int timeout = 60;
		
		//System.out.println("=========> " + getClass().getResource("/mqtt.properties"));
		//System.out.println("=========> " + getClass().getResource("/org/eclipse/paho/client/mqttv3/ca/ca.crt"));
    	...
    	if(isTlsCheck)
		{
			...
			keepalive = optionsComp.getKeepAlive() != keepalive ? keepalive : optionsComp.getKeepAlive();
			String cafile = null, clicrtfile = null, clikeyfile = null, password = null;
			java.io.Reader cafileRder = null, clicrtfileRder = null, clikeyfileRder = null;
			
			
			try
			{
				final Properties mqttprps = getMQTTPrps();	
				//String prpKey = null;
				//for(java.util.Enumeration keyPrps = mqttprps.propertyNames(); keyPrps.hasMoreElements();)
		        //{
		        //	prpKey = (String)keyPrps.nextElement();
		        //	
		        //	System.out.println(prpKey+"==> "+mqttprps.getProperty(prpKey) );
		        //}
				
				//if(mqttprps.contains("tls.timeout"))
					timeout = Integer.valueOf(mqttprps.getProperty("tls.timeout")).intValue();
				
				//if(mqttprps.contains("tls.keep.alive"))
					keepalive = Integer.valueOf(mqttprps.getProperty("tls.keep.alive")).intValue();
					
				//if(mqttprps.contains("tls.server.ca"))
					cafile = mqttprps.getProperty("tls.server.ca");
				
				//if(mqttprps.contains("tls.client.ca"))
					clicrtfile = mqttprps.getProperty("tls.client.ca");
				
				//if(mqttprps.contains("tls.client.key"))
					clikeyfile = mqttprps.getProperty("tls.client.key");
					
				//if(mqttprps.contains("tls.client.password"))
					password = mqttprps.getProperty("tls.client.password");
				...
				opts.setSocketFactory(
					org.eclipse.paho.sample.mqttv3app.SslUtil.getSocketFactory(
						cafileRder, 
						clicrtfileRder, 
						clikeyfileRder, 
						password
    ...
    public void connect( String connStr, boolean usePersistence ) throws MqttException {
		...
		opts = getMqttConnectOptions(chkTls.isSelected());
     ...
     */
   	public void actionPerformed( ActionEvent e) {
		...
		setTitleText( "" );
   		...
   		else if ( e.getActionCommand().equals("SecureConnection") ) {
   			//chkTls.setSelected(true); 
