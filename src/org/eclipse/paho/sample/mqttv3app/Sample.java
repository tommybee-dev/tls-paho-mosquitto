...
public class Sample implements MqttCallback {
	...
	public static void main(String[] args) {
		...
		String cafile = null;
		String clicrtfile = null;
		String clikeyfile = null;
		...
		for (int i=0; i<args.length; i++) {
			...
			if (args[i].length() == 2 && args[i].startsWith("-")) {
				...
				switch(arg) {
					...
					case 'e': cafile = args[++i];                 break;
					case 'f': clicrtfile = args[++i];             break;
					...
					case 'l': ssl = Boolean.valueOf(args[++i]).booleanValue(); break;
					case 'v': tlsmode = Boolean.valueOf(args[++i]).booleanValue(); break;
					case 'u': userName = args[++i];               break;
					case 'y': clikeyfile = args[++i];             break;
    ...
    String url = protocol + broker + ":" + port;
		...
		try {
			...
			Sample sampleClient = null;
			
			if(tlsmode)
				sampleClient = new Sample(url, clientId, cleanSession, quietMode,userName, password, cafile,clicrtfile,clikeyfile);
			else
				sampleClient = new Sample(url, clientId, cleanSession, quietMode,userName, password);
			
			System.out.println("tlsmode.. " + tlsmode + " ...connection to " + url);
     ...
     * @param cafile whether debug should be printed to standard out
     * @param clicrtfile the username to connect with
     * @param clikeyfile the password for the user
	 * @throws MqttException
	 */
	Sample(String brokerUrl, String clientId, boolean cleanSession, boolean quietMode, String userName, String password,String cafile,String clicrtfile,String clikeyfile)
	{
    	...
    	try {
			...
			if(cafile != null)
			{
				...
				try
				{
					conOpt.setSocketFactory(SslUtil.getSocketFactory(cafile, clicrtfile, clikeyfile, password));
				}catch(Exception e)
				{
					e.printStackTrace();
				}
    ...
    public Sample(String brokerUrl, String clientId, boolean cleanSession, boolean quietMode, String userName, String password) throws MqttException {
    	this(brokerUrl, clientId, cleanSession, quietMode, userName, password,null,null,null);
    }
	   ...
	   static void printHelp() {
	      System.out.println(
	          "Syntax:\n\n" +
	              ...
	              "     \n\n TLS Options \n" +
	              "    -v  TLS/SSL enabled; true - (default is false) " +
	              "    -e  CA certification file with openssl generally\n" +
	              "    -f  Client certification file \n" +
	              "    -y  Client key file\n" +
	              "     \n\n SSL Options \n" +
	              "    -l  SSL enabled; true - (default is false) " +
